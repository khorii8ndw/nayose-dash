...
def page_layout(
    ...
    *,
    ...
    - Container: 横幅・左右余白を管理
    - Stack: 縦に「上 / 真ん中 / 下」を並べる
    - ScrollArea: 真ん中だけスクロール
    """
    return dmc.Container(
        className="page-root",   # ← ここを CSS の起点にする
        size=container_size,
        px=px,
        py=py,
        children=dmc.Stack(
...
layout = page_layout(
    top=dmc.Title("単一名寄せ", order=2),
    middle=dmc.Group(
        align="flex-start",
        grow=True,
        children=[contents, contents2],
    ),
